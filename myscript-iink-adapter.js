...
(function () {
  ...
  // ponytail: dev-only. When served from localhost, pull the optional
  // dev-keys.js (serve.sh writes it from .env) so any applet pointed at this
  // adapter uses those keys. Absent file is fine.
  function loadDevKeys() {
    return new Promise(function (resolve) {
      if (!/^https?:\/\/(localhost|127\.0\.0\.1)(:|\/)/.test(IINK_BASE)) {
        return resolve();
      }
      var script = document.createElement('script');
      script.src = IINK_BASE + 'dev-keys.js';
  ...
  function loadAssets() {
    ...
    assets = loadDevKeys().then(function () {
  ...
  function configuration(settings) {
    settings = settings || {};
    // ponytail: dev-only override, set by dev-keys.js (generated by serve.sh from .env)
    var dev = window.MYSCRIPT_DEV_KEYS || {};
    return {
      configuration: {
        server: {
          scheme: 'https',
          host: 'cloud.myscript.com',
          applicationKey: settings.applicationKey || dev.applicationKey || DEFAULT_APPLICATION_KEY,
          hmacKey: settings.hmacKey || dev.hmacKey || DEFAULT_HMAC_KEY
        },
        recognition: {
          'raw-content': {
